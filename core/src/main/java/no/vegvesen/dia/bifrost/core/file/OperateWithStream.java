...
public class OperateWithStream implements FileOperation {

    private static final Logger LOGGER = LogManager.getLogger(OperateWithStream.class);

    private final String contentType;
    private final ServiceContextIF serviceContextIF;

    public OperateWithStream(String contentType, ServiceContextIF serviceContextIF) {
    ...
    @Override
    public ResponseEntity<InputStreamResource> fetchObject(String bucket, String path) {
        try {
            // Big nono to use as resource, that will terminate the stream before response is received.
            final InputStream is = context().s3client()
